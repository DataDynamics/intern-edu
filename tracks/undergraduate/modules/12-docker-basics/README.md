...
데이터 엔지니어는 데이터베이스, 워크플로우 도구(Airflow), 메시지 시스템(Kafka) 같은 것들을 거의 항상 컨테이너로 띄웁니다. 이 모듈에서는 앞서 SQLite로 익힌 SQL을, 이번엔 **컨테이너로 띄운 진짜 PostgreSQL**에서 실행해 봅니다. 실습에는 Docker Desktop(또는 Docker Engine)이 필요합니다.
