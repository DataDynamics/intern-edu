...
`exercises/`에서 **CSV를 읽어 집계한 뒤 JSON으로 저장하는** 작은 파이프라인을 만듭니다. 주문 CSV를 딕셔너리 목록으로 읽고, 금액을 숫자로 바꾸되 불량 값은 건너뛰며, 카테고리별 매출을 집계해 보기 좋은 JSON으로 저장하는 것이 과제입니다. 그 흐름은 다음과 같습니다.

```mermaid
flowchart LR
    A["orders.csv"] --> B["load<br/>DictReader로 읽기"]
    B --> C["clean<br/>금액 숫자화·불량 건너뛰기"]
    C --> D["aggregate<br/>카테고리별 매출 합계"]
    D --> E["summary.json"]
```

이것은 사실 "추출→변환→적재"라는 ETL의 축소판으로, 졸업생 트랙에서 본격적으로 다루는 파이프라인의 씨앗입니다. `python check.py`가 통과하면 완성입니다.
